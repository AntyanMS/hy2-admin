...
Исходники панели — каталог **`panel/`**, сборка бинаря: `tools/build-panel-linux.sh` (см. [Releases](https://github.com/AntyanMS/hy2-admin/releases)).
...
На вкладке **Direct routing** (релиз **v0.0.6** и новее) можно подтянуть список доменов из внешнего открытого репозитория и применить их к правилам **direct** (включая IP по DNS). Суффиксы **`.ru` / `.рф` / `.su`** выбираются чипами с флагами (только Россия); по умолчанию включены все три.
